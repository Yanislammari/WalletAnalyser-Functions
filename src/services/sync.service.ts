...
class SyncService {
  ...
  public async syncAssetPrices(limit: number): Promise<void> {
    ...
    for (const asset of assets) {
      try {
        const latestPrice = await this.marketstackService.fetchLatestPrice(asset.ticker_name!);
        ...
        const existingAssetPrice: AssetPrice = await this.assetPriceRepository.getByAssetAndDate(asset.uuid, priceDate);
